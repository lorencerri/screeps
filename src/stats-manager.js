...
const StatsManager = {
	runForAllRooms: function () {
		...
		var roomStats = myRooms.map((room) => {
			...
			const spawns = Object.keys(room.find(FIND_MY_SPAWNS));
			var spawnsUsage = spawns.length ? _.sum(spawns.map((i) => (i.spawning ? 1 : 0))) / spawns.length : 0;
